...
% endeff = transl(0.7,0.1,0.2);
% p560.ikine(endeff,qn,[1,1,1,0,0,0])
q = [0, deg2rad(45), deg2rad(-80), 0, deg2rad(45), 0];
% 
p560.fkine(q)
% 
...
% 
% point1co = p560.fkine(q)
% point2co = p560.fkine(q) * transl(0,0,1)
% point1 = [0.8730,0.0786,-0.0203];
% point2 = [1.8389,0.3374,-0.0203];
% normal = [-1,0,0];
% point = [1.8,0,0];
% LinePlaneIntersection(normal,point,point1,point2)

%p560.ikine(transl(0.6,0.1,0.1),qn,[1,1,1,0,0,0])
%ballans = inv(p560.fkine(q)) * ball
%p560.plot(q,'workspace',workspace,'scale',scale);
%p560.fkine(q)
%p560.fkine(q)*transl(0,0,10)
%LinePlaneIntersection([-1,0,0],[1.8,0,0],p560.fkine(q),p560.fkine(q)*transl(0,0,10))
%clear L
%point2 = p560.fkine(q) * transl(0,0,1)
%normal = [-1, 0, 0]